...
png(file = 'Plots/N1forest_SFD.png', width = 1600, height = 1200, units = "px")
...
studies[26]<- "Angele et al. (2013c)"
studies[22]<- "Angele et al. (2013c), Exp.2"
studies[8]<- "Rayner et al. (2014b)"
studies[2]<- "Veldre et al. (2015a)"; studies[21]<- "Veldre et al. (2015b)"
studies[16]<- "Yang et al. (2012a)"; studies[14]<- "Yang et al. (2012b)";
studies[15]<- "Yang et al. (2012b), Exp.2"
# observed:
obsM<- data$T
...
# posterior:
pM<- unname(sum41$statistics[3:nrow(sum41$statistics),1])
CrI_L<- unname(sum41$quantiles[3:nrow(sum41$statistics),1])
CrI_R<- unname(sum41$quantiles[3:nrow(sum41$statistics),5])
...
#  windows()
  ...
  # plot y grid
  for (i in 1:nrow(df)){
    segments(x0=start+step*i, y0=-70, x1=start+step*i, y1=maxY+27.8, lwd=1.2, lty=1, col="#E3E3E3")
  }
...
for (i in 1:nrow(df)){
  ...
  # posterior mean:
#  abline(h=sum41$statistics[1,1], lwd=1.2, lty=1, col="darkred")
  
# pooled mean:
  points(x= (97.3/100)*maxX, y=sum41$statistics[1,1], pch = 18, cex=2.2, col="darkred")
  segments(x0=(97.3/100)*maxX, y0=sum41$quantiles[1,1], x1=(97.3/100)*maxX, y1=sum41$quantiles[1,5], lwd=1.2, lty=1, col="darkred")
  segments(x0=(97.3/100)*maxX-5, y0=sum41$quantiles[1,5], x1=(97.3/100)*maxX+5, y1=sum41$quantiles[1,5], lwd=1.2, lty=1, col="darkred")
  segments(x0=(97.3/100)*maxX-5, y0=sum41$quantiles[1,1], x1=(97.3/100)*maxX+5, y1=sum41$quantiles[1,1], lwd=1.2, lty=1, col="darkred")
  
  abline(h=140.5)

 #  legend:
  rect(xleft = (98.7/100)*maxX, ybottom = 68, xright = (95.7/100)*maxX, ytop = 135, lwd=1.2, lty = "solid", col="white") 
 #  observed:
  segments(x0=(96.5/100)*maxX, y0=72, x1=(96.5/100)*maxX, y1=84, lwd=1.2, lty=2, col="black")
  points(x= (96.5/100)*maxX, y=77.4, pch = 15, cex=1.3, col="white")
  points(x= (96.5/100)*maxX, y=77.4, pch = 0, cex=1.3, col="black")

 # posterior
  points(x= (97.8/100)*maxX, y=77.4, pch = 15, cex=1.5, col="darkred")
  segments(x0=(97.8/100)*maxX, y0=72, x1=(97.8/100)*maxX, y1=84, lwd=1.2, lty=1, col="darkred")
  
  # text:
  text(x=(97.8/100)*maxX, y=88, "Posterior (95% CrIs)", cex=1.2, font=2, srt = 90, adj=0)
  text(x=(96.5/100)*maxX, y=88, "Observed (95% CIs)", cex=1.2, font=2, srt = 90, adj=0)
